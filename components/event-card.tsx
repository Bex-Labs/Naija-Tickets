...
export function EventCard({
  ...
  return (
    <article
      className={`group relative overflow-hidden border border-[#241b3f]/10 bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:border-emerald-400/50 hover:shadow-[0_22px_60px_rgba(66,42,94,.14)] ${compact ? 'grid grid-cols-[7.5rem_minmax(0,1fr)]' : 'block'}`}
    >
      {event.id && (
        <EventSaveButton
          eventId={event.id}
      ...
      <a
        ...
        className={compact ? 'contents' : 'block'}
      >
        <div
        ...
        >
          ...
          {event.soldOut && (
            <span
              className={`absolute bg-emerald-500 px-2 py-1 text-[10px] font-bold text-emerald-950 ${compact ? 'bottom-2 left-2' : 'bottom-4 left-4'}`}
            >
              Sold out
            </span>
          )}
        </div>
        <div className={compact ? 'p-3.5 pt-14' : 'p-5'}>
          <p
